...
import { getLlmopsOverview, getRuntimeSettings } from "@/lib/api";
...
export default async function SettingsPage() {
  const [overview, settings] = await Promise.all([getLlmopsOverview(), getRuntimeSettings()]);
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="space-y-5 xl:sticky xl:top-[92px] xl:self-start">
          <Card><CardHeader><SectionHeading eyebrow="Guardrails" title="Budget and safety" /></CardHeader><CardContent>
            ...
            <div className="mt-2 flex justify-between font-mono text-[9px] text-muted"><span>Current usage</span><span>{latestEval.avg_cost_usd ? `$${Number(latestEval.avg_cost_usd).toFixed(2)} avg/run` : "No usage recorded"}</span></div>
